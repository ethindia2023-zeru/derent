...
export async function testOwnerFunctions(deployer: any, testToken: Contract) {
  const [_deployer, owner, tenant] = await ethers.getSigners();
  ...
  // tenant pays rent
  // owner claim rent
  await testToken.connect(deployer).addListing(advance, securityDeposit, rent, waitingPeriod, false, location);
  ...
  // await testToken.connect(owner).addListing(advance, securityDeposit, rent, waitingPeriod, false, location);
  // console.log("added the listing");
  ...
  // await testToken.connect(deployer).bidOnProperty(3, parseEther("11"));
